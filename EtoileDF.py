...
class Etoile (object): 
    def __init__(self, modele, source):
        ...
        elts = ["X","2H","3He","Y","6Li","7Li","7Be","9Be","10B","11B",
            ...
            "30Si","31P","32S","33S","34S","35S","35Cl","36S","36Cl","37Cl","heavy"] #liste de tous les éléments considérés | n crée des bugs et devra sûrement être supprimé
        ...
        if modele == "Starevol" : #PROBLEME D'INDICE, PREND UNE COLONNE A GAUCHE, ARRIVE PAS A FIX
            ...
            while 1 :
                ...
                if  float(ligne[12])>1e4 and (">\n" not in ligne) and ("<\n" not in ligne) and ("@\n" not in ligne):
                    ...
                    self.R.append(float(ligne[5]))
                    self.t.append(float(ligne[12])) #N'est plus en log
        ...
        for s in self.args.keys() : #Ajoute toutes les valeurs au dataframe
            try :
                self.DF[s] = self.args[s]
            except :
                pass
    ...
    def Evolution (self, Varx = "t", Vary = [], xlegend = "xlegend", ylegend = "ylegend", logx = False, logy = False, masse = False, Zini = False) : 
        ...
        if type(Vary) != list : print("Erreur") #Verifie le bon type de données
        ...
        for i in Vary : #Trace tout les graphes
            ...
            Ne marche pas et je doute de l'utilité

            if logy == True :
                loogy = []
                for z2 in range(len(self.DF[i])) :
                    #loogy.append(log(self.DF[i].iloc[z2], 10))
                    print(self.DF[i].iloc[z2])
                i = "log(" + i + ")"
                print("Okay")
                self.DF[i] = loogy'''
    ...
    def Test (self):
        print(self.DF["Y_coeur"])


if __name__ == "__main__" :

    Struct = Structure(modele = "Genec", source = "T:\Cours\DATA\GENEC\classique_m1.0.v1")
    Star = Etoile(modele = "Genec", source = "T:\Cours\DATA\GENEC\classique_m1.0.dat")
    Star.Evolution(Vary = ["X_coeur", "Y_coeur"], logx = True, logy = True)
